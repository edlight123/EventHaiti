...
export async function POST(request: NextRequest) {
  try {
    ...
    const organizerEmail = String(body?.organizerEmail || 'info@edlihgt.org').trim().toLowerCase()
    const organizerName = String(body?.organizerName || 'EventHaiti Organizer').trim() || 'EventHaiti Organizer'

    const publish = body?.publish !== false
    const batchTag = String(body?.batchTag || `seed20_${new Date().toISOString()}`).slice(0, 80)
