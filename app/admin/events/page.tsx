              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="hidden md:table-cell px-6 py-3 text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider">
                      Date
                    </th>
                    <th className="hidden sm:table-cell px-3 sm:px-6 py-2.5 sm:py-3 text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider">
                      Price
                    </th>
                    <th className="px-3 sm:px-6 py-2.5 sm:py-3 text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider">
                      Status
                  ...
                  {eventsWithOrganizers.map((event: any) => (
                    <tr key={event.id} className="hover:bg-gray-50">
                      <td className="px-3 sm:px-6 py-3 sm:py-4">
                        <div className="text-[13px] sm:text-sm font-medium text-gray-900 line-clamp-1">{event.title}</div>
                        <div className="text-[11px] sm:text-[13px] text-gray-500 line-clamp-1">{event.venue_name || event.city}</div>
                      </td>
                      <td className="px-3 sm:px-6 py-3 sm:py-4">
                        <div className="text-[13px] sm:text-sm text-gray-900">
                          {event.organizer?.full_name || 'Unknown'}
                        </div>
                        <div className="hidden sm:block text-[11px] sm:text-[13px] text-gray-500 line-clamp-1">{event.organizer?.email}</div>
                      </td>
                      <td className="hidden md:table-cell px-6 py-4 text-[13px] text-gray-500 whitespace-nowrap">
                        {event.start_datetime ? new Date(event.start_datetime).toLocaleDateString() : 'N/A'}
                      </td>
                      <td className="hidden sm:table-cell px-3 sm:px-6 py-3 sm:py-4 text-[13px] sm:text-sm text-gray-900 whitespace-nowrap">
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-[13px] sm:text-sm font-medium">
                        <a
