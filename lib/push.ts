...
export async function subscribeToPush(publicKey: string, topics: string[] = [], userId?: string) {
  ...
  if (existing) {
    // Merge topics for existing subscription
    await fetch('/api/push/subscribe', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ endpoint: existing.endpoint, keys: (existing as any).keys, topics, userId })
    })
    return existing
  }
  ...
  await fetch('/api/push/subscribe', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ endpoint: sub.endpoint, keys: (sub as any).keys, topics, userId })
  })
  return sub
}
