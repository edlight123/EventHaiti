    ...
    <Link href={`/events/${event.id}`} className="group">
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-soft hover:shadow-hard transition-all duration-300 overflow-hidden border border-gray-100 group-hover:-translate-y-1.5 group-hover:border-brand-200 relative">
        ...
        {event.banner_image_url ? (
          ...
          <div className="h-40 sm:h-52 bg-gradient-to-br from-brand-100 via-brand-50 to-accent-100 flex items-center justify-center relative group-hover:from-brand-200 group-hover:to-accent-200 transition-all duration-500">
            <span className="text-3xl sm:text-5xl group-hover:scale-110 transition-transform duration-500">🎉</span>
        ...
        <div className="p-3 sm:p-5">
          <div className="flex items-start justify-between mb-2 sm:mb-3 gap-2">
            ...
            {event.users?.is_verified && (
              <div className="inline-flex items-center gap-1 text-blue-600 bg-blue-50 px-2 py-1 rounded-lg" title="Verified Organizer">
          ...
          <h3 className="text-sm sm:text-base font-semibold text-gray-900 mb-1.5 line-clamp-2 group-hover:text-brand-700 transition-colors duration-300 leading-snug">
            {event.title}
          </h3>

          <p className="text-xs sm:text-sm text-gray-600 mb-2.5 sm:mb-3 line-clamp-2 leading-relaxed">
            {event.description}
          </p>

          {/* Tags */}
          {event.tags && event.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3 overflow-hidden max-h-8">
          ...
          <div className="space-y-1.5 mb-3 sm:mb-4">
            <div className="flex items-center text-[13px] sm:text-sm text-gray-700">
              <div className="w-7 h-7 rounded-lg bg-accent-50 flex items-center justify-center mr-2 flex-shrink-0">
          ...
          <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-gray-100">
            <div>
              <p className="text-[11px] text-gray-500 mb-0.5 font-medium">From</p>
              {isFree ? (
                <p className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-success-600 to-success-700 bg-clip-text text-transparent">FREE</p>
              ) : (
                <p className="text-lg sm:text-2xl font-bold text-gray-900">
                  {event.ticket_price} <span className="text-[11px] sm:text-xs font-normal text-gray-600">{event.currency}</span>
                </p>
              )}
            </div>
            {!isSoldOut && (
              <div className="text-right">
                <p className="text-[11px] text-gray-500 mb-0.5 font-medium">Available</p>
                <p className={`text-[11px] sm:text-xs font-bold ${selloutSoon ? 'text-warning-600' : 'text-brand-700'}`}>
